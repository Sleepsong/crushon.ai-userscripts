...
(function () {
    ...
    async function getChatMetadata() {
        ...
        for (const block of messageBlocks) {
            const modelSpan = block.querySelector('button span');
            if (modelSpan && modelSpan.innerText.trim()) { model = modelSpan.innerText.trim(); break; }
        }
        ...
        if (characterDetailsAnchor) {
            const href = characterDetailsAnchor.getAttribute('href');
            if (href) {
                character_url = `https://crushon.ai${href}`;
            }
            const imgElement = characterDetailsAnchor.querySelector('img');
            if (imgElement) {
                originalImageUrl = imgElement.getAttribute('src') || '';
            }
        } else {
            const genericImg = document.querySelector('div[class*="avatar"] img, img[alt*="avatar"]');
            if (genericImg) {
                originalImageUrl = genericImg.getAttribute('src') || '';
            }
            console.warn("[Chat Scraper] Could not find primary character details link for character_url. URL might be missing.");
        }

        if (originalImageUrl.includes('optimize/https://')) {
            originalImageUrl = originalImageUrl.split('optimize/')[1].split('?')[0];
        }
        const imageBase64 = await fetchImageAsBase64(originalImageUrl);

        const authorLink = document.querySelector('a[href^="/profile/"]');
        const authorName = authorLink?.innerText.trim() || 'Unknown';
        const authorUrl = authorLink ? `https://crushon.ai${authorLink.getAttribute('href')}` : '';

        let sceneBase64 = null;
        const sceneContainer = document.querySelector('div.absolute.inset-0.-z-10.size-full');
        if (sceneContainer) {
            const imgs = sceneContainer.querySelectorAll('img');
            if (imgs.length > 1 && imgs[1]?.getAttribute('src')) {
                 sceneBase64 = await fetchImageAsBase64(imgs[1].getAttribute('src'));
            } else if (imgs.length > 0 && imgs[0]?.getAttribute('src')) {
                 sceneBase64 = await fetchImageAsBase64(imgs[0].getAttribute('src'));
            }
        }
        let intro = null;
        const introSection = Array.from(document.querySelectorAll('span')).find(s => s.textContent.trim()==='Intro')?.closest('div')?.parentElement?.parentElement;
        if (introSection) {
            const introText = introSection.querySelector('.rich-html')?.textContent?.trim();
            if (introText) intro = introText;
        }
        let tags = [];
        const tagContainer = document.querySelector('div.flex.flex-wrap.gap-1');
        if (tagContainer) {
            tags = Array.from(tagContainer.querySelectorAll('div > span, a > span'))
    ...
    function createDownloadButton() {
        ...
        const button = document.createElement('div'); button.id = BUTTON_ID; button.className = 'flex w-full cursor-pointer items-center backdrop-blur-[2px] rounded-lg px-3 py-2 sm:dark:hover:bg-white/20 sm:hover:bg-black/10';
        const inner = document.createElement('div'); inner.className = 'flex w-full items-center justify-center';
        const icon = document.createElementNS('http://www.w3.org/2000/svg', 'svg'); icon.setAttribute('width', '16'); icon.setAttribute('height', '16'); icon.setAttribute('viewBox', '0 0 24 24'); icon.setAttribute('fill', 'none'); icon.classList.add('size-5'); icon.innerHTML = `<path d="M5 20h14v-2H5v2zm7-18L5.33 9h3.67v6h4V9h3.67L12 2z" fill="currentColor"/>`;
        const label = document.createElement('div'); label.className = 'ml-2 w-full max-w-[88px] truncate'; label.textContent = 'Download';
        inner.appendChild(icon); inner.appendChild(label); button.appendChild(inner);
            ...
            const messages = Array.from(document.querySelectorAll('.MarkdownText_CustomMarkdownText__P3bB6')).map(msg => {
                ...
                if (messageBlock) {
                    ...
                    if (explicitSpeakerElement && explicitSpeakerElement.innerText.trim()) {
                        ...
                        if (hasModelButton) { speaker = mainAiNameForFile; } else { speaker = secondCharacterName; } // Use secondCharacterName if not main AI and no tag
                    }
                }
            ...
            if (format === '1') {
                ...
                try {
                    ...
                    const jsonOutput = {
                        ...
                        author: { name: metadata.authorName, url: metadata.authorUrl },
                        ...(metadata.sceneBase64 ? { sceneImage: metadata.sceneBase64 } : {}),
                    ...
                    a.href = url; a.download = `${mainAiNameForFile}_chat_log.json`; a.click();
        ...
        const observer = new MutationObserver(() => { const toolsPanel = Array.from(document.querySelectorAll('div')).find(div => div.textContent?.includes('Instruction') && div.className.includes('flex') && div.className.includes('flex-col') && !div.className.includes('gap-')); if (toolsPanel && !document.getElementById(BUTTON_ID)) { toolsPanel.appendChild(button); } else if (!toolsPanel && document.getElementById(BUTTON_ID)) { document.getElementById(BUTTON_ID).remove(); }});
        observer.observe(document.body, { childList: true, subtree: true });
    }

    function observeUntilReady() {
        const readyObserver = new MutationObserver((mutations, obs) => { const container = document.querySelector('#chat-container') || document.querySelector('div[class*="ChatRoom_chatPanel"]'); const messagesExist = document.querySelector('.MarkdownText_CustomMarkdownText__P3bB6'); if (container && messagesExist) { createDownloadButton(); obs.disconnect(); }});
        readyObserver.observe(document.body, { childList: true, subtree: true });
    }
    let lastUrl = location.href;
    const urlObserver = new MutationObserver(() => {
        const currentUrl = location.href; if (currentUrl !== lastUrl) { lastUrl = currentUrl; const oldButton = document.getElementById(BUTTON_ID); if (oldButton) oldButton.remove(); if (window.location.pathname.includes("/chat")) { observeUntilReady(); }}
    });
    urlObserver.observe(document.body, { childList: true, subtree: true });
    if (window.location.pathname.includes("/chat")) { if (document.readyState === 'complete') setTimeout(observeUntilReady, 500); else window.addEventListener('load', () => { observeUntilReady(); }); }
